...
public class DefaultCharacterSelectionScreen : CharacterSelectionScreen {
	...
	public override void SetHoverIndex(int player, int characterIndex){
		if (!this.closing){
			this.p1HoverIndex = Mathf.Clamp(this.p1HoverIndex,0,Mathf.Min(this.selectableCharacters.Length,this.characters.Length)-1);
			this.p2HoverIndex = Mathf.Clamp(this.p2HoverIndex,0,Mathf.Min(this.selectableCharacters.Length,this.characters.Length)-1);
			base.SetHoverIndex(player, characterIndex);

			if(
				characterIndex >= 0 && 
				characterIndex < this.selectableCharacters.Length && 
